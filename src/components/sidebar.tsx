...
import { NAVIGATION_ITEMS, SECONDARY_NAV_ITEMS } from "@/lib/constants/navigation";
  ...
  return (
    <aside className="hidden sm:flex flex-col w-64 h-dvh sticky top-0 border-r border-slate-800 bg-slate-950/50 backdrop-blur-xl z-50">
      ...
      <div className="flex-1 px-4 py-2 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          if (item.isScanner) {
            return (
              <QRScanner 
                key="sidebar-scanner"
                customTrigger={
                  <button className="flex items-center w-full gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/5 transition-all group">
        ...
        <div className="pt-6">
          <p className="px-2 mb-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Lainnya</p>
